...
const BinarySwitchSupport = (superClass) => class extends superClass {
  ...
  binarySwitchTurnOnOff (onOff, instance = 1) {
    this.zwave.setValue(this.node.nodeid, 37, instance, 0, !!onOff)
  }

  binarySwitchInvert (instance = 1) {
    const state = this.binarySwitchGetState(instance)
    this.zwave.setValue(this.node.nodeid, 37, instance, 0, !state)
  }

  binarySwitchGetState (instance = 1) {
    return this.node.classes[37][0][instance].value
  }
