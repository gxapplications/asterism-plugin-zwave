...
class PhilioPse04 extends UnknownProduct.with(
  ...
  playTone (tone) {
    this.zwave.setValue(this.node.nodeid, 32, 1, 0, tone || 0) // BASIC COMMAND CLASS !
  }
  ...
  getVolume () {
    return this.node.classes[121][2][1].value
  }

  play (tone, volume) {
    if (volume !== undefined) {
      if (this._previousVolume === null) {
        this._previousVolume = this.getVolume()
        this.setVolume(volume)
        const duration = this.getConfiguration(PhilioPse04.meta.configurations.SOUND_DURATION) * 30000
        this.playTone(tone)
        setTimeout(() => { // restore previous volume after sound played.
...
PhilioPse04.meta = {
  ...
  settingPanelProvidedFunctions: ['getName', 'getLocation', 'setName', 'setLocation', 'getConfiguration', 'setConfiguration',
    ...
    'playTone', 'setTone', 'setVolume', 'getVolume', 'play'],
  configurations: {
