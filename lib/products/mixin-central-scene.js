    ...
    switch (comClass) {
      case 91: // 0x5B
        ...
        if (filter !== undefined) {
          ...
          if (filtered !== null) {
            try {
              this.logger.info(`Node #${this.node.nodeid} central scene triggered: ${value.value} -> {${filtered.toString()}}.`)
            } catch (err) {
              this.logger.info(`Node #${this.node.nodeid} central scene triggered: ${value.value}.`)
            }
