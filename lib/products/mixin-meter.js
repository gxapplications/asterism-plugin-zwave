...
const MeterSupport = (index) => (superClass) => class extends superClass {
  ...
  _meterStackData (value) {
    this.dataMeter.push({ t: Date.now(), v: value.value, i: value.instance || 1 })
    while (this.dataMeter.length > 1024) {
      this.dataMeter.shift()
    }
    this.dataHandler.setItem(this.dataMeterKey, this.dataMeter)
    // TODO !5: Exploit data in an item.
  }

  meterResetCounter (instance = 1) {
    this.zwave.pressButton(this.node.nodeid, 50, instance, 257) // TODO !0: bug to reset meter: https://github.com/OpenZWave/node-openzwave-shared/issues/337
