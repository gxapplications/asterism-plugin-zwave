...
const EnergyConsumptionMeterSupport = (meterIndex = 0, instantEnergyInstance = 1, meterInstance = 1) => (superClass) => class extends superClass {
  constructor (node, context) {
    ...
    this.dataConsumptionMeter = null
    this.dataHandler.getItem(this.dataConsumptionMeterKey)
      .then((data) => {
        this.dataConsumptionMeter = (data || []).concat(this.dataConsumptionMeter || [])
      })
  ...
  _meterStackData (value) {
    this.scenariiService.getEnergyPricing()
      .catch(() => {
        console.log('Energy pricing seems to be undefined. Must set it first!')
        return 0
      })
      .then((pricing) => {
        const lastConsumptionMeasure = (this.dataConsumptionMeter && this.dataConsumptionMeter.length > 0)
        ...
        if (this.dataConsumptionMeter === null) {
        ...
        while (this.dataConsumptionMeter.length > 700) { // 1024 causes overflows in db rows
          this.dataConsumptionMeter = this.dataConsumptionMeter.slice(-700)
        }
