...
export default class ServerZwaveTemperatureTrigger extends ServerTrigger {
  constructor (data) {
    super(data)
    ...
    ServerZwaveTemperatureTrigger.zwaveService.context.zwave.on('value changed', (nodeId, comClass, values) => {
      if (comClass === 49 && values.index === 1) {
    ...
    ServerZwaveTemperatureTrigger.zwaveService.context.zwave.on('scan complete', () => {
      setTimeout(() => {
        const node = ServerZwaveTemperatureTrigger.zwaveService.getNodeById(this.data.nodeId)
  ...
  reschedule () {
    ...
    if (this.listeners.filter((l) => !l.lazy).length === 0) {
      ServerZwaveTemperatureTrigger.logger.info(`Trigger ${this.data.name} has no active listener. Trigger unscheduled.`)
