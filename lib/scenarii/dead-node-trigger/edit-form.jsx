...
class ZwaveDeadNodeTriggerEditForm extends React.Component {
  constructor (props) {
    super(props)
    ...
    return ready ? (
      <Row className='section card form'>
        {compatibleNodes.length > 0 ? instance.data.nodeIds.map((nodeId, idx) => (
          <Input key={uuid.v4()} s={12} m={6} l={4} label={`Z-wave device #${idx + 1}`} type='select' icon='blur_circular'
        ...
        <Input key={uuid.v4()} s={12} m={6} l={4}
          label={`Z-wave device #${instance.data.nodeIds.length + 1}`} type='select' icon='blur_circular'
  ...
  nameChange () {
    if (this.props.instance.data.nodeIds.length === 0) {
      this.props.instance.data.name = 'Misconfigured dead node trigger'
      return
    }
...
ZwaveDeadNodeTriggerEditForm.propTypes = {
...
ZwaveDeadNodeTriggerEditForm.defaultProps = {
  highlightCloseButton: () => {}
}

ZwaveDeadNodeTriggerEditForm.label = 'Z-wave dead node trigger'

export default ZwaveDeadNodeTriggerEditForm
