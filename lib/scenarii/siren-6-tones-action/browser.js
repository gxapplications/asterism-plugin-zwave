...
class BrowserZwaveSiren6TonesAction extends BrowserAction {
  ...
  get shortLabel () {
    const defaultName = `Misconfigured siren control on nodes ${this.nodesList()}`
    return this.data.name ? `Triggers siren(s) ${this.data.name} with tone '${this.data.tone}'` : defaultName
  }
  get fullLabel () {
    const defaultName = `Misconfigured siren control on nodes ${this.nodesList()}`
    return this.data.name ? `Triggers Z-wave sirens for ${this.data.name} with tone '${this.data.tone}'.` : defaultName
  }
