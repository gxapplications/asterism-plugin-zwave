...
class ZwaveService {
  ...
  updateAlarmsToNotify (alarmsToUpdate) {
    ...
    alarmsToUpdate.forEach(({ node, status, value }) => {
      ...
      alarmIdsToUpdate.push(key)
    })

    // TODO !1: notifs are then better now ?
    const alarmsOn = Object.entries(this.alarms).filter(([key, alarm]) => alarm.alarmIsOn || alarmIdsToUpdate.includes(key))
