...
const _getProductClass = (node) => {
  ...
  return products[`${manufacturerid}-${producttype}-${productid}`] || node.overrideProduct || UnknownProduct
}
...
class ZwaveService {
  constructor ({ getServices, zwave, dataHandler, logger, privateSocketIo, sendRefreshNeededSignal, updateNotification }) {
    ...
    zwave.on('node event', this._otherEvent.bind(this, 1, this.context))
  ...
  _classValueChanged (nodeId, comClass, value) {
    ...
    if (node) {
      ...
      this.logger.log(`Zwave 'value added/changed events' for node #${nodeId}: ${JSON.stringify(lightValue)}.`)
      return true
    }
  ...
  _nodeReady (ready, context, nodeId, nodeInfo) {
    ...
    if (node) {
      ...
      if (ready !== false) {
        ...
        if (ready === undefined) {
          this.logger.info(`Zwave 'node ready event': node #${nodeId} ready state unknown.`)
        }
  ...
  _otherEvent (numb, context, nodeId, data) {
    ...
    if (node) {
      ...
      this.logger.log(`Zwave '??? event' for node #${nodeId}: ${numb} / ${data}`)
      return true
    }

    throw new Error(`Zwave '??? event': the nodeId #${nodeId} was not found! ${data}.`)
  }
  ...
  persistConfiguration () {
    ...
    try {
      //this.context.zwave.writeConfig() // TODO !1: see if needed, this is deprecated: replace or remove it !
    } catch (error) {
  ...
  updateAlarmsToNotify (alarmsToUpdate) {
    ...
    this.updateNotification(alarmsOn.length > 0)
  }
