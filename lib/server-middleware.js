...
const zwaveServerMiddleware = (context) => {
  ...
  const updateNotification = () => {
    notificationHandler.setNotification(
      'controller-state',
      (_currentConnectionState <= 1) ? 'zwave-off' : 'zwave-on',
      (_currentConnectionState <= 0) ? 'error' : '',
      () => {
        if (context.privateSocketIo) {
          if (_currentConnectionState < 2) {
            context.privateSocketIo.emit('notification-open-settings')
          } else {
            context.privateSocketIo.emit('notification-open-edit-panel')
          }
        }
        return 'ok'
      }
  ...
  const _plugSocketListeners = (socket) => {
    ...
    socket.on('get-node-settings-panel-provided-functions', (nodeId, ack) => {
      ack(zwaveService.getNodeSettingPanelProvidedFunctions(nodeId))
    })
    socket.on('call-node-public-function', (nodeId, functionName, args, ack) => {
      zwaveService.callNodeProvidedFunction(nodeId, functionName, args)
