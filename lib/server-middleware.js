...
const zwaveServerMiddleware = (context) => {
  ...
  /* TODO !0: bug fix for alarms
      When alarm turns on, alarms is: [
        [ '3-113-1-6', { alarmIsOn: true, status: ['Access Control'], node: [Object] } ]
      ]
      When alarm turns OFF, alarms is empty array
   */
  const updateNotification = (alarms = undefined) => {
    if (alarms !== undefined) {
      alarmsToNotify = alarms.filter(([key, alarm]) => {
        const name = alarm.node.loc ? `${alarm.node.name} @ ${alarm.node.loc}` : alarm.node.name
        notificationHandler.pushNotificationMessage(
          `From Asterism: Z-Wave alarm`,
          `Alarm ${alarm.alarmIsOn ? 'raised' : 'ended'} on ${name}: [${alarm.status.join(', ')}].`,
...
console.log('###0 BEFORE filter, then AFTER: ', alarms, alarmsToNotify)
