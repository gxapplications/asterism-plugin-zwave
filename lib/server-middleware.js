  ...
  const updateNotification = (alarms = undefined) => {
    ...
    if (alarms !== undefined) {
      alarmsToNotify = alarms.filter(([key, alarm]) => { // Warning: side effect filter!
        ...
        console.log('#########1', alarm.node.getLocation, alarm.node.location)
        console.log('#########2', alarm.node)
        const name = (alarm.node.getLocation && alarm.node.getLocation())
          ? `${alarm.node.name} @ ${alarm.node.getLocation()}`
          : (alarm.node.location ? `${alarm.node.name} @ ${alarm.node.location}` : alarm.node.name)
